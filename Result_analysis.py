import pandas as pd
import numpy as np
Diff_genes = pd.read_csv("Data/Diff_data/diff_ctr_TC.csv",index_col=0).index.tolist()

diff_prot = pd.read_csv("Data/Diff_labels/luminal protein estrus.csv",index_col=0).index.tolist()
...
gene_names = pd.read_csv("Data/Data_cpm/Data_control.csv",index_col=0).index.tolist()
...
def replace_value(cell_value):
        ...
        return cell_value
for i in list1:
    TF_high_prot_2_5 = pd.read_csv("Tfs_allprot_"+i+".csv")
    if i!="control":
        geneexpbasedprot = pd.read_csv("Siggenebasedprotlist_TCT"+i+".csv",header=None)[0].to_list()
    ...
    filtered_df3.to_csv("AllTfs_diffcodinggene_"+i+".csv",index=0)