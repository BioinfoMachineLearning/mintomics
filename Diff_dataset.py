...
def gene2protein(stage = 'train', size = 10, pertage = 0.5):
    ...
    Gene_file = ['diff_ctr_TC.csv', 'diff_ctr_T0.5.csv', 'diff_ctr_T1.5.csv']
    Pro_file = ['luminal protein estrus.csv', 'luminal protein 0.5.csv', 'luminal protein 1.5.csv']
    ...
    for dat, tar in zip(Gene_all, Prot_all):
        ...
        P_lar = tar[tar > 0.5]
        tar_clss = torch.as_tensor([1 if x>0.5 else 0 for x in tar])
...
class Psedu_data(Dataset):
    def __init__(self, n = 10):
        ...
        for i in range(n):
            ...
            feature = torch.randn(16100, 7)
            label = torch.zeros(1, 3000)
