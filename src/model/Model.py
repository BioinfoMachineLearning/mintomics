...
class TransformerMintomics(nn.Module):
    def __init__(self, dim_model=7, attn_head=1, dim_ff=32, drop=0.5, batch_f=True, encoder_layers=1,n_class=1):
        ...
        self.encod = TransformerEncoderLayer(d_model=dim_model, nhead=attn_head, dim_feedforward=dim_ff, dropout=drop, 
                                                                    batch_first=batch_f)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = src
        if self.norm_first:
            x,attent = x + self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, is_causal=is_causal)
            x = x + self._ff_block(self.norm2(x))
        else:
            x1,attent = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)
            x = self.norm1(x +x1 )
